variable "storage_table_url" {
  description = "The Storage Share URL (not the resource_id) in which this file will be placed into, eg https://alzmgttfstatenznprdst.table.core.windows.net/globalOutputs"
  type        = string
}

variable "writes" {
  description = <<DESCRIPTION
Configuration for writing outputs to the GlobalOutputs table.
One entity is created per module call (partition_key + row_key).

Example:
  writes = {
    partition_key = "alz-platform-connectivity-hub"
    row_key       = coalesce(var.hub_name, local.region.cli_name)
    outputs = {
...
variable "reads" {
  description = <<DESCRIPTION
Configuration for reading outputs from the GlobalOutputs table.
